...
var messages = [
  ...
  'character', //个字数
  'no image', //没有图片
  'current image url is error<br>is to check next?', //当前图片地址异常<br>是否继续查看下一张？
  'next', //下一张
  'no look',  //不看了
]
  ...
  var code = setInterval(function() {
    if ($.i18n && $.i18n.map) {
       messages.forEach(function(item, i) {
          var key = item.split(' ').splice(0, 2).join('_')
          messages[i] = $.i18n.map['layer_' + key]
       })
       clearInterval(code)
    }
  }, 1000)
  ...
  return layer.open($.extend({
    ...
    success: function(layero){
      ...
      if(value === ''){
        ...
        layer.tips(messages[3]+ (options.maxlength || 500) +messages[4], prompt, {tips: 1});
      } else {
...
layer.photos = function(options, loop, key){
  ...
  loadImage(data[start].src, function(img){
    ...
    layer.close(dict.loadi);
    layer.msg(messages[6], {
      time: 30000, 
      btn: [messages[7], messages[8]], 
      yes: function(){
