(function ($) {
    ...
    function bindData(template, data, settings) {
        ...
        processElements("data-content-html", template, data, settings, function ($elem, value) {
            $elem.html(applyFormatters($elem, value, "content", settings));
        });
        ...
        processElements("data-for", template, data, settings, function ($elem, value) {
            $elem.attr("for", applyFormatters($elem, value, "for", settings));
        });
        ...
        processElements("data-list", template, data, settings, function ($elem, value) {
            var value = applyFormatters($elem, value, "value", settings);
            ...
            if (value && value.length) {
                ...
                value.forEach(function(item) {
                    for (var i in item) {
                        switch (i) {
                            case 'href':
                                $('[data-list-'+ i +']', $elem).attr(i, item[i])
                                break
                            default:
                                $('[data-list-'+ i +']', $elem).html(item[i])
                        }
                    }
                    html += $elem.html()
                })
